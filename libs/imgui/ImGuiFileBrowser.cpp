...
#include <GLFW/glfw3.h> // (CUSTOM -- for close on escape)

namespace imgui_addons
{
  ImGuiFileBrowser::ImGuiFileBrowser(const std::string &initialPath)
    {
  ...
  bool ImGuiFileBrowser::showFileDialog(const std::string& label, const DialogMode mode, const ImVec2& sz_xy, const std::string& valid_types)
    {
        ...
        if (ImGui::BeginPopupModal(label.c_str(), nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            // (CUSTOM) close if escape pressed
            if(ImGui::IsKeyPressed(GLFW_KEY_ESCAPE)) { closeDialog(); }
    ...
    //Unix only
    #ifndef OSWIN
  void ImGuiFileBrowser::initCurrentPath(const std::string &initialPath)
    {
        ...
        char* real_path = realpath((initialPath.empty() ? "./" : initialPath.c_str()), buffer);

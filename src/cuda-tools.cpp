  ...
  if(!gCudaInitialized || (devId > 0 && devId != gDevId))
    {
      ...
      std::cout<< "========================================================================================\n"
               << "== INITIALIZING CUDA...\n"
               << "========================================================================================\n"
               << "====   Number of GPUs with CUDA capability detected: " << devCount << "\n";
      
      if(devCount == 0) { std::cout << "========> WARNING(cuda): No devices found that support CUDA.\n\n"; return false; }
      devId = std::max(devId, 0);
      if(devId > devCount-1) { std::cout << "========> ERROR(cuda): Device " << devId << " is not a valid CUDA device.\n\n"; return false; }
      ...
      std::cout << "====   CUDA device [" << deviceProps.name << "] has " << deviceProps.multiProcessorCount << " Multi-Processors\n";

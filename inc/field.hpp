...
{
  // field components
  float4 Qcol  = float4{1.0f, 0.0f, 0.0f, 1.0f}; // base colors
  float4 Ecol  = float4{0.0f, 1.0f, 0.0f, 1.0f};
  float4 Bcol  = float4{0.0f, 0.0f, 1.0f, 1.0f};
  float  Qmult = 0.2f; float Emult = 0.2f; float Bmult = 0.2f; // additional multipliers
  // materials
  float4 epCol   = float4{1.0f, 0.0f, 0.0f, 1.0f}; // base colors
  float4 muCol   = float4{0.0f, 1.0f, 0.0f, 1.0f};
  float4 sigCol  = float4{0.0f, 0.0f, 1.0f, 1.0f};
  float  epMult  = 0.2f; float muMult  = 0.2f; float sigMult = 0.2f; // additional multipliers
  // 2D parameters
  int   numLayers2D = 1; // blends layers from numLayers to 0 (top-down)
  // 3D parameters
  float opacity = 0.1f; float brightness = 2.0f;
};

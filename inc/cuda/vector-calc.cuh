...
//// GRADIENT --> ∇[T]
template<typename T, typename VT3=typename cuda_vec<T, 3>::VT, typename IT3=typename cuda_vec<VT3>::IT>
...
//// DIVERGENCE --> ∇·[VT3]
// (from cell center)
template<typename VT3, typename T=typename cuda_vec<VT3>::BASE, typename IT3=typename cuda_vec<VT3>::IT>
...
// jacobi iteration
template<typename VT3, typename T=typename cuda_vec<VT3>::BASE, typename IT3=typename cuda_vec<VT3>::IT>
__device__ VT3 jacobi(const Field<VT3> &src, const IT3 &p, T alpha, T beta)
{
  const VT3 v0  = src[src.idx(p)];
  const VT3 vNX = src[src.idx(p.x-1, p.y, p.z)]; const VT3 vNY = src[src.idx(p.x, p.y-1, p.z)]; const VT3 vNZ = src[src.idx(p.x, p.y, p.z-1)];
  const VT3 vPX = src[src.idx(p.x+1, p.y, p.z)]; const VT3 vPY = src[src.idx(p.x, p.y+1, p.z)]; const VT3 vPZ = src[src.idx(p.x, p.y, p.z+1)];
...
{
  const VT3 v0  = src[src.idx(p0)];
  const VT3 vNX = ((p1n.x >= 0 && p1p.x >= 0) ? src[src.idx(p1n.x, p0.y,  p0.z )] : makeV<VT3>(0));
  const VT3 vNY = ((p1n.y >= 0 && p1p.y >= 0) ? src[src.idx(p0.x,  p1n.y, p0.z )] : makeV<VT3>(0));
  const VT3 vNZ = ((p1n.z >= 0 && p1p.z >= 0) ? src[src.idx(p0.x,  p0.y,  p1n.z)] : makeV<VT3>(0));
  const VT3 vPX = ((p1n.x >= 0 && p1p.x >= 0) ? src[src.idx(p1p.x, p0.y,  p0.z )] : makeV<VT3>(0));
  const VT3 vPY = ((p1n.y >= 0 && p1p.y >= 0) ? src[src.idx(p0.x,  p1p.y, p0.z )] : makeV<VT3>(0));
  const VT3 vPZ = ((p1n.z >= 0 && p1p.z >= 0) ? src[src.idx(p0.x,  p0.y,  p1p.z)] : makeV<VT3>(0));
  return (vNX+vPX + vNY+vPY + vNZ+vPZ + v0*alpha)*beta;
}
...
// jacobi iteration
template<typename VT3, typename T=typename cuda_vec<VT3>::BASE, typename IT3=typename cuda_vec<VT3>::IT>
__device__ T jacobi(const Field<T> &src, const IT3 &p, T alpha, T beta)
{
  const T v0  = src[src.idx(p)];
  const T vNX = src[src.idx(p.x-1, p.y, p.z)]; const T vNY = src[src.idx(p.x, p.y-1, p.z)]; const T vNZ = src[src.idx(p.x, p.y, p.z-1)];
  const T vPX = src[src.idx(p.x+1, p.y, p.z)]; const T vPY = src[src.idx(p.x, p.y+1, p.z)]; const T vPZ = src[src.idx(p.x, p.y, p.z+1)];
  return (vNX+vPX + vNY+vPY + vNZ+vPZ + v0*alpha)*beta;
}
// jacobi iteration
template<typename VT3, typename T=typename cuda_vec<VT3>::BASE, typename IT3=typename cuda_vec<VT3>::IT>
...
{
  const T v0  = src[src.idx(p0)];
  const T vNX = ((p1n.x >= 0 && p1p.x >= 0) ? src[src.idx(p1n.x, p0.y,  p0.z )] : 0.0);
  const T vNY = ((p1n.y >= 0 && p1p.y >= 0) ? src[src.idx(p0.x,  p1n.y, p0.z )] : 0.0);
  const T vNZ = ((p1n.z >= 0 && p1p.z >= 0) ? src[src.idx(p0.x,  p0.y,  p1n.z)] : 0.0);
  const T vPX = ((p1n.x >= 0 && p1p.x >= 0) ? src[src.idx(p1p.x, p0.y,  p0.z )] : 0.0);
  const T vPY = ((p1n.y >= 0 && p1p.y >= 0) ? src[src.idx(p0.x,  p1p.y, p0.z )] : 0.0);
  const T vPZ = ((p1n.z >= 0 && p1p.z >= 0) ? src[src.idx(p0.x,  p0.y,  p1p.z)] : 0.0);
  return (vNX+vPX + vNY+vPY + vNZ+vPZ + v0*alpha)*beta;
}

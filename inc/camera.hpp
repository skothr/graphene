...
void Camera<T>::rotate(VT2 angles) // angles --> { pitch, yaw, roll } (rotation around x, y, and z bases)
{
  ...
  Vector<T, 3> newDir   = normalize(Vector<T, 3>(dir));
  if(angles.x != 0.0)   { newDir = normalize(::rotate(newDir, Vector<T,3>(upBasis), angles.x)); }
  Vector<T, 3> newRight = normalize(cross(newDir, Vector<T,3>(upBasis)));
  if(angles.y != 0.0)   { newDir = normalize(::rotate(newDir, newRight, angles.y)); }
  Vector<T, 3> newUp    = normalize(cross(newRight, newDir));

  Vector<T, 3> newPos   = pos;
  if(angles.x != 0.0)   { newPos = ::rotate(newPos, Vector<T,3>(upBasis), angles.x); }
  Vector<T, 3> right2   = normalize(cross(newPos, newUp));
  if(angles.y != 0.0)   { newPos = ::rotate(newPos, right2, -angles.y); }
  
  dir = to_cuda(newDir); right = to_cuda(newRight); up = to_cuda(newUp); pos = to_cuda(newPos);
}
